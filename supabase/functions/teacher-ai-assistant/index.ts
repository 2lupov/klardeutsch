...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const lang: string = body.lang === "uk" ? "uk" : "ru";
    ...
    const langName = lang === "uk" ? "українською мовою" : "російською мовою";
    const langStrict = lang === "uk"
      ? "ВІДПОВІДАЙ ВИКЛЮЧНО УКРАЇНСЬКОЮ МОВОЮ. Не використовуй російську мову взагалі."
...
⚙️ ПРАВИЛА ОФОРМЛЕННЯ ВІДПОВІДІ:
- 🌍 МОВА ВІДПОВІДІ: ${langName}. ${langStrict}
- Завжди використовуй красиве **Markdown-форматування**:
  • \`## Заголовки\` для розділів
  • **жирний** для ключових ідей, *курсив* для перекладів
  • Списки (-, 1.) для перелічення
  • > цитати для важливих правил/прикладів (📌 Regel, 💡 Tipp, ⚠️ Achtung, 📖 Beispiel)
  • Markdown-таблиці \`| ... | ... |\` для відмінювання
  • \`code\` для окремих німецьких слів
- Німецькі приклади виділяй **жирним**, переклад — *курсивом*.
- Структуруй відповідь: спочатку коротка суть, потім приклади/таблиці, потім порада.
- Будь конкретним — викладач читає прямо на уроці.
- Якщо просять вправу — давай готову вправу + відповідь + коротке пояснення.
